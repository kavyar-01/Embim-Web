    ...
    <div class="alert alert-error">
      ...
      <?php
        $errorMessages = [
          ...
          'invalid' => 'Invalid review selected.',
          'delete_failed' => 'Failed to delete review. It may already be gone.',
        ];
      ?>
      <?= htmlspecialchars($errorMessages[$_GET['error']] ?? 'An error occurred.') ?>
    </div>
  <?php endif; ?>
      ...
      <?php foreach ($reviews as $r): ?>
        <div class="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm relative group hover:shadow-md transition-shadow">
          ...
          <div class="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity flex gap-2">
            <a href="?page=edit_review&id=<?= $r['id'] ?>" class="p-1.5 bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white rounded-lg transition-colors" title="Edit Review">
